...
import numpy as np
from predictor.config import TARGET_COLUMN
...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        try:
            ...
            logging.info(f"Selecting column name which contains null above the {threshold}")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df = utils.convert_columns_int(df=base_df, exclude_columns=exclude_columns)
            train_df = utils.convert_columns_int(df=train_df, exclude_columns=exclude_columns)
            test_df = utils.convert_columns_int(df=test_df, exclude_columns=exclude_columns)
